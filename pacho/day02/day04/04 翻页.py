...
def getJob(url):
    ...
    for job in jobList:
        ...
        print(jobName)
        jobHref = job.xpath('./p[1]/span[1]/a/@href')[0]
        # 公司名
        company = job.xpath('./p[1]/a/text()')[0]
        # 地址
        address = job.xpath('./p[1]/span[2]/text()')[0]
        # 薪资
        money = job.xpath('./p[1]/span[3]/text()')
        if len(money) !=0:
            money = money[0]
        else:
            money = '面议'
...
if __name__ == '__main__':
    ...
    for cityName, cityHref in cityDict.items():
        totaPage = getPage(cityHref)
        for i in range(1, totaPage + 1):
            # 拼接url
            url = cityHref + "p%d" % i
            # 写入文本
